...
2. Open **Settings → PropertyMe Sync**:
   ...
   - Four scopes are requested: `property:read` (`/lots`), `contact:read` (`/members`), `activity:read` (`/v1/inspections`) and `offline_access`. `communication:read` and `transaction:read` were removed in 2026-08 (messages and financial transactions the site has no use for). Saved settings migrate automatically on the next admin page load; a scope change only takes effect at the next authorisation, so disconnect and reconnect after one.
...
- Only four OAuth scopes are requested — `property:read` (`/lots`), `contact:read` (`/members`), `activity:read` (`/v1/inspections` — PropertyMe files inspections under Activities, per its OpenAPI spec), and `offline_access`. The communication and transaction scopes are deliberately not requested: they would grant access to messages and financial data (tenant ledgers, owner statements, rent payments) the site has no use for. Saved settings are migrated automatically — retired scopes removed, `activity:read` restored where an earlier version stripped it.
- **A scope change never affects the current token** — refresh tokens keep the grant they were issued with. To pick up a scope change, disconnect and reconnect. (This is also why a wrong narrowing can pass local testing: an old token still carries the old scopes.)
